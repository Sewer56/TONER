...
namespace HeroesONE_R.Utilities
{
    ...
    public static class StructUtilities
    {
        ...
        /// <param name="bytes">The array of bytes to convert into a specified structure.</param>
        /// <returns>The array of bytes converted to the user's own specified class or structure.</returns>
        public static unsafe TStructure MemoryOffsetToPointer<TStructure>(ref byte[] bytes)
        {
            fixed (byte* ptr = &bytes[0])
        ...
        /// <param name="offset">The offset into the structure from which to obtain the structure from.</param>
        /// <returns>The array of bytes converted to the user's own specified class or structure.</returns>
        public static unsafe TStructure MemoryOffsetToPointer<TStructure>(ref byte[] bytes, int offset)
        {
            fixed (byte* ptr = &bytes[offset])
                return Unsafe.Read<TStructure>(ptr);
        }
        ...
        /// <param name="pointer">A number that becomes incremented by the size of the structure when it is read.</param>
        /// <returns>The array of bytes converted to the user's own specified class or structure.</returns>
        public static unsafe TStructure MemoryOffsetToPointer<TStructure>(ref byte[] bytes, int offset, ref int pointer)
        {
            fixed (byte* ptr = &bytes[offset])
            {
        ...
        /// <summary>
        /// Returns a pointer to a structure in memory by incrementing the specified pointer by the offset and
        /// increments a specified pointer by the size of the specified structure.
        /// </summary>
        /// <typeparam name="TStructure">A user specified class or structure to convert an array of bytes to.</typeparam>
        /// <param name="filePointer">The pointer to start of a file or structure the offset is from.</param>
        /// <param name="fileOffset">The offset into the structure for which the address is to be returned.</param>
        /// <param name="pointer">A number that becomes incremented by the size of the structure when it is read.</param>
        /// <returns>The array of bytes converted to the user's own specified class or structure.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe void* MemoryOffsetToPointer<TStructure>(IntPtr filePointer, int fileOffset, ref int pointer)
        {
            pointer += Unsafe.SizeOf<TStructure>();
            return (void*)(filePointer + fileOffset);
        }


        /// <summary>
        /// Returns a pointer to a structure in memory by incrementing the specified pointer by the offset.
        /// </summary>
        /// <typeparam name="TStructure">A user specified class or structure to convert an array of bytes to.</typeparam>
        /// <param name="filePointer">The pointer to start of a file or structure the offset is from.</param>
        /// <param name="fileOffset">The offset into the structure for which the address is to be returned.</param>
        /// <returns>The memory offset returned as a pointer.</returns>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static unsafe void* MemoryOffsetToPointer<TStructure>(IntPtr filePointer, int fileOffset)
        {
            return (void*)(filePointer + fileOffset);
        }
