...
namespace HeroesONE_R.Structures.SonicHeroes
{
    public unsafe struct ONEArchive
    {
        ...
        public static ONEArchive ParseONEFile(ref byte[] file)
        {
            ...
            oneArchive.FileHeader = StructUtilities.MemoryOffsetToPointer<ONEHeader>(ref file, pointer, ref pointer);
            oneArchive.FileNameSectionHeader = StructUtilities.MemoryOffsetToPointer<ONEFileNameSectionHeader>(ref file, pointer, ref pointer);
            ...
            for (int x = 0; x < fileNameCount; x++)
                oneArchive.FileNames[x] = StructUtilities.MemoryOffsetToPointer<ONEFileName>(ref file, pointer, ref pointer);
            ...
            while ((pointer < file.Length) && (oneArchive.Files.Count < fileCount))
            {
                ...
                // Parse the header.
                oneFile.ONEFileHeader = StructUtilities.MemoryOffsetToPointer<ONEFileHeader>(ref file, pointer, ref pointer);
